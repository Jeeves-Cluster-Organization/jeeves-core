...
def test_quota_fields_match():
    """Rust ResourceQuota fields match Python _dict_to_quota_defaults() reads."""
    # quota_to_value() uses serde_json::to_value(q) where q is ResourceQuota,
    # so we extract fields from the ResourceQuota struct in kernel/types.rs.
    types_file = HANDLER_DIR.parent.parent / "kernel" / "types.rs"
    rust_source = types_file.read_text(encoding="utf-8")
    py_source = KERNEL_CLIENT.read_text(encoding="utf-8")

    rust_fields = _extract_dto_struct_fields(rust_source, "ResourceQuota")
