...
class IntentClassifier:
    ...
    def get_thresholds(self) -> Dict[str, float]:
        ...
        return {
            ...
            'journal_threshold': self.journal_threshold,
    ...
    def update_thresholds(
        ...
        journal_threshold: Optional[float] = None,
        fact_threshold: Optional[float] = None
    ) -> None:
        """
        Update classification thresholds.

        Args:
            task_threshold: New task threshold (0.0-1.0)
            journal_threshold: New journal threshold (0.0-1.0)
            fact_threshold: New fact threshold (0.0-1.0)
        """
        if task_threshold is not None:
            self.task_threshold = max(0.0, min(1.0, task_threshold))
        if journal_threshold is not None:
            self.journal_threshold = max(0.0, min(1.0, journal_threshold))
        if fact_threshold is not None:
            self.fact_threshold = max(0.0, min(1.0, fact_threshold))

        self._logger.info(
            "thresholds_updated",
            task=self.task_threshold,
            journal=self.journal_threshold,
            fact=self.fact_threshold
        )
