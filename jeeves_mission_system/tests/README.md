...
**Location**: `jeeves_mission_system/tests/`
...
pytest jeeves_mission_system/tests/contract -v
...
pytest jeeves_mission_system/tests/unit -m "not requires_llamaserver and not requires_postgres"
...
pytest jeeves_mission_system/tests/integration -v
...
# Run E2E tests
pytest jeeves_mission_system/tests -m e2e
...
```bash
pytest jeeves_mission_system/tests/unit \
...
```bash
pytest jeeves_mission_system/tests/integration \
...
```bash
pytest jeeves_mission_system/tests -m e2e -v
...
- **`envelope_factory`** - Creates envelopes via `jeeves_mission_system.contracts`
...
def test_core_engine_no_mission_imports():
    ...
    assert not any(
        "jeeves_mission_system" in imp for imp in core_imports
...
Note: Rate limiting is handled by `jeeves_control_tower` - see `jeeves_control_tower/resources/rate_limiter.py`.
...
# Watch mode
pytest-watch jeeves_mission_system/tests/unit
```
...
**Foundation Layers**:
- **L0: jeeves_protocols** - Protocol definitions, InterruptKind enum, Envelope
- **L0: jeeves_shared** - Shared utilities (logging via `get_component_logger`, serialization, UUID)
- **L1: jeeves-airframe** - Inference platform substrate (backend adapters for HTTP, SSE, retries)
- **L3: jeeves_avionics** - Infrastructure orchestration (LLM providers delegate to Airframe)
