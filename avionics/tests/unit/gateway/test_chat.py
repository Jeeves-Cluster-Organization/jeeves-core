...
import pytest
from avionics.gateway.routers.chat import (
...
class TestEventCategoryClassification:
    ...
    def test_exact_match_agent_lifecycle_generic(self):
        """Test exact match for generic agent lifecycle events."""
    ...
    def test_exact_match_agent_lifecycle_specific(self):
        """Test exact match for specific agent lifecycle events."""
        # Standardized naming: agent.<component>.<action>
        assert _classify_event_category("agent.perception.started") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.perception.completed") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.intent.started") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.intent.completed") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.planner.started") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.planner.completed") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.traverser.started") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.traverser.completed") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.synthesizer.started") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.synthesizer.completed") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.integration.started") == EventCategory.AGENT_LIFECYCLE
        assert _classify_event_category("agent.integration.completed") == EventCategory.AGENT_LIFECYCLE

    def test_exact_match_critic_decision(self):
        """Test exact match for critic decision events."""
        assert _classify_event_category("agent.critic.started") == EventCategory.CRITIC_DECISION
        assert _classify_event_category("agent.critic.decision") == EventCategory.CRITIC_DECISION
    ...
    def test_event_category_map_completeness(self):
        ...
        expected_event_types = [
            ...
            # Specific agent lifecycle (standardized naming)
            "agent.perception.started", "agent.perception.completed",
            "agent.intent.started", "agent.intent.completed",
            "agent.planner.started", "agent.planner.completed",
            "agent.traverser.started", "agent.traverser.completed",
            "agent.synthesizer.started", "agent.synthesizer.completed",
            "agent.integration.started", "agent.integration.completed",
            # Critic
            "agent.critic.started", "agent.critic.decision",
            # Tools
