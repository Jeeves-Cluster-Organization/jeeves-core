...
class MissionSystemAdapters:
    ...
    def memory(self) -> Any:
        ...
        if not self._memory_service:
            from memory_module.services.memory_service import MemoryService
...
def create_database_client(
    ...
    Constitutional compliance:
        Apps access infrastructure via adapters, not direct avionics imports.
    """
    from avionics.database.factory import create_database_client as _create_db
    if settings is None:
...
def get_settings() -> SettingsProtocol:
    ...
    Constitutional compliance:
        Apps access infrastructure via adapters, not direct avionics imports.
    """
    from avionics.settings import get_settings as _get_settings
...
def get_feature_flags() -> Any:
    ...
    Constitutional compliance:
        Apps access infrastructure via adapters, not direct avionics imports.
    """
    from avionics.feature_flags import get_feature_flags as _get_flags
    return _get_flags()
...
def create_tool_executor(registry: Optional[ToolRegistryProtocol] = None) -> Any:
    ...
    """
    from avionics.wiring import create_tool_executor as _create_executor
