...
This constitution defines the rules for **control_tower**—the kernel layer that sits between the Gateway and Mission System, managing request lifecycle, resource allocation, service dispatch, and event streaming.
...
Control Tower **ONLY imports from** `protocols` and `shared`:

```python
# CORRECT: Import from protocols
from protocols import Envelope, ServiceProtocol
from protocols import InterruptKind, RateLimitConfig  # Interrupt/rate limit types
from shared import get_component_logger  # Shared logging utilities
...
and rate limit types (RateLimitConfig, RateLimitResult) are defined in `protocols.interrupts`
...
**InMemoryCommBus** (`control_tower/ipc/commbus.py`):
...
```python
from control_tower.ipc import get_commbus, InMemoryCommBus
...
- **Mission System:** [../mission_system/CONSTITUTION.md](../mission_system/CONSTITUTION.md)
- **Avionics:** [../avionics/CONSTITUTION.md](../avionics/CONSTITUTION.md)
